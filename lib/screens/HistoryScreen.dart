...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Future<void> getDataKeHoach(String? token, String start, String end) async {
    ...
    try {
      List<ChuyenHangModel> datas = await GetAction.getChuyenHangDaXacNhan(token ?? "",start, end);
      setState(() {
        ChuyenHangs = datas;
      });
      ...
      if (ChuyenHangs[i].soThe!.trim().toLowerCase().contains(textSearch.toLowerCase().trim())||
        ...
        DataRow listItemRow = DataRow(
          cells: [
            ...
            DataCell(Padding(
              padding: const EdgeInsets.symmetric(horizontal: 3),
              child: Text(ChuyenHangs[i].trangThaiThe ?? ""),
            )),
            DataCell(Padding(
              padding: const EdgeInsets.symmetric(horizontal: 3),
    ...
    if (loadding) {
      ...
      return CustomScaffoldLayout(
        ...
        children: Flex(
          ...
          children: [
            Text("DANH SÁCH CÁC CHUYẾN ĐÃ XÁC NHẬN", style: TextStyle(color: Colors.blue, fontWeight: FontWeight.w600, fontSize: 18),),
                ...
                child: SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    columns: const [
                      ...
                      DataColumn(label: Center(child: Text('Trạng thái', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)))),
